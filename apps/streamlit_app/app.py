...
with tab4:
    ...
    if sp_df.empty and other_df.empty:
        ...
        st.divider()
        st.subheader("🌡️ Density view (Kepler.gl)")
        st.caption(
            "Discovery dataset (other states), rendered with Kepler.gl for a richer "
            "dark-themed density view. Loaded on demand — the exported HTML is a "
            "self-contained ~13 MB bundle."
        )
        if KEPLER_HTML.exists():
            if st.button("Load Kepler.gl map"):
                with st.spinner("Loading Kepler.gl (large embedded bundle)..."):
                    kepler_html = KEPLER_HTML.read_text(encoding="utf-8")
                components.html(kepler_html, height=650, scrolling=True)
        else:
            st.info(
                f"Kepler.gl export not found at `{KEPLER_HTML}`. Generate it with "
                "`yolo_training_exp3.ipynb` (Kepler section) and commit it to that path."
            )
