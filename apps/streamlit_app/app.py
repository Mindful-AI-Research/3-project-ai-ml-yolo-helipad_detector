...
# A descriptive User-Agent and a fixed timeout: some public APIs (OpenSky included, per
# community reports) apply stricter anti-abuse filtering to requests carrying the default
# "python-requests/x.y" User-Agent and/or originating from cloud/datacenter IP ranges
# (which is exactly what Streamlit Community Cloud runs on). Identifying the client
# properly is a legitimate, low-risk mitigation regardless of which factor is at play.
OPENSKY_HEADERS = {"User-Agent": "HelipadDetector-Dashboard/1.0 (+https://github.com/Mindful-AI-Research/3-project-ai-ml-yolo-helipad_detector)"}
OPENSKY_TIMEOUT = (5, 12)  # (connect, read) seconds
OPENSKY_MAX_ATTEMPTS = 2   # one retry on transient connect/read failures before giving up
...
def _opensky_auth():
    ...
    st.secrets (see README's "Live Helicopter Traffic" setup note), use them.
    Authenticated requests get a higher daily quota (~4000 vs ~400) and, per
    OpenSky's own guidance, are less likely to be caught by anti-abuse filtering
    aimed at anonymous datacenter/cloud traffic — a plausible fix if the anonymous
    endpoint keeps timing out specifically when deployed on Streamlit Cloud.
    Returns None (anonymous request) if no credentials are configured, so the
    layer keeps working exactly as before for anyone who hasn't set this up.
    """
    try:
        user = st.secrets.get("opensky", {}).get("username")
        pwd = st.secrets.get("opensky", {}).get("password")
    except Exception:
        return None
...
def fetch_live_helicopter_flights(bbox: dict) -> tuple[list[dict], str | None]:
    ...
    hiccup, timeout, or exhausted quota degrades this layer gracefully (empty
    list + short error string) instead of crashing the tab. Cached for 30s so
    normal Streamlit reruns don't burn through OpenSky's request quota.
    ...
    codes (e.g. 403/429), since those won't resolve by repeating the same
    request immediately.
    """
    payload = None
    last_error = None
    for attempt in range(OPENSKY_MAX_ATTEMPTS):
        try:
            resp = requests.get(
                OPENSKY_STATES_URL, params=bbox, headers=OPENSKY_HEADERS,
                timeout=OPENSKY_TIMEOUT, auth=_opensky_auth(),
            )
