...
MODEL_ROOT = Path("artifacts/runs/runs/detect")  # note the doubled "runs/runs" — see README repo structure
SAMPLES_DIR = Path("data/samples")
COORDS_CSV = Path("src/geospatial/helipad_coordinates_bbox.csv")
EXEC_REPORT_EN = Path("reports/executive_analysis/🇬🇧Helipoint_Detector_Model_Performance_and_Data_Analysis.pdf")
EXEC_REPORT_PT = Path("reports/executive_analysis/🇧🇷Helipoint_Detector_Model_Performance_and_Data_Analysis.pdf")
...
def discover_models() -> dict[str, Path]:
    ...
    if MODEL_ROOT.exists():
        for exp_dir in sorted(MODEL_ROOT.iterdir()):
...
def load_experiment_metrics() -> pd.DataFrame:
    """Scans artifacts/runs/detect/*/results.csv and builds a comparison table
    with the best epoch of each experiment found (today: exp1; exp2 shows up
    on its own as soon as it exists, no code changes needed)."""
    rows = []
...
def load_experiment_curves() -> dict[str, pd.DataFrame]:
    ...
    per-epoch metric evolution chart."""
    curves = {}
    if not MODEL_ROOT.exists():
        return curves
    for exp_dir in sorted(MODEL_ROOT.iterdir()):
...
@st.cache_data(show_spinner=False)
def load_helipad_locations() -> pd.DataFrame:
    """Reads src/geospatial/helipad_coordinates_bbox.csv and computes the
    center point (lat, lon) of each bounding box, for the map view."""
...
def get_selected_model() -> YOLO:
    label = st.session_state.get("model_choice", next(iter(MODEL_OPTIONS)))
...
with st.sidebar:
    ...
    if not MODEL_OPTIONS:
        st.error(
            "No model found at `artifacts/runs/runs/detect/*/weights/best.pt`. "
            "Train at least one experiment before running the app."
        )
        st.stop()
    ...
    st.selectbox(
        ...
        key="model_choice",
        help="Every folder under artifacts/runs/detect/ with a ready best.pt shows up here "
...
st.caption(f"Active model: **{st.session_state.get('model_choice', list(MODEL_OPTIONS)[0])}**")
...
with st.expander("📊 Experiment Metrics", expanded=True):
    if metrics_df.empty:
        st.info("No `results.csv` found yet under `artifacts/runs/runs/detect/*/`.")
    else:
...
with tab4:
    st.subheader("🗺️ Helipad Locations")
    st.caption(
        "Center point of each geographic bounding box collected by the Selenium automation "
        "(`src/geospatial/helipad_scraper.py`) and converted in "
        "`src/geospatial/transform_coordinates.py`."
    )

    locations_df = load_helipad_locations()

    if locations_df.empty:
        st.info(
            f"No coordinates found at `{COORDS_CSV}`. Run the scraper and the coordinate "
            "conversion script first (see the Execution Guide)."
        )
    else:
        st.write(f"**{len(locations_df)} helipad(s) plotted.**")
